# dataminded-presentation-operator
...
`kubectl port-forward example-pod 8082:80`
...
```sh
make docker-build docker-push IMG=<some-registry>/dataminded-presentation-operator:tag
```
	
3. Deploy the controller to the cluster with the image specified by `IMG`:

```sh
make deploy IMG=<some-registry>/dataminded-presentation-operator:tag
```
